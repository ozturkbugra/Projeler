...
namespace SirketProje
{
    public partial class Gelirler : Form
    {
        ...
        {
            string sql = "Insert Into tblGelir (Aciklama,Tutar,SirketID,Tarih) values (@p1,@p2,@p3,@p4) ";
            conn.Open();
            SqlCommand cmd = new SqlCommand(sql, conn);
            ...
            cmd.ExecuteNonQuery();
            conn.Close();
            MessageBox.Show("Gelir oluşturuldu");
            dgvGider.DataSource = b.veriAl("Select ID,Aciklama,Tutar,Tarih from GelirView where SirketID= " + CbSirket.SelectedValue + "");
        }

        private void button2_Click(object sender, EventArgs e)
        {
            string sql = "delete from tblGelir where ID=@p1 ";
            conn.Open();
            SqlCommand cmd = new SqlCommand(sql, conn);
